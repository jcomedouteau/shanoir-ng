...
begin
    ...
    left join users.events ev on (ev.object_id = ex.id and ev.event_type = 'importDataset.event')
    left join users.users us on ev.user_id = us.id
